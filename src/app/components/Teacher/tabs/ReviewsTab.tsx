...
import { Star, CheckCircle2, ListFilter } from "lucide-react";
...
const ReviewsTab = ({ teacherId, setCacheReviews, cacheReviews }): any => {
  const [page, setPage] = useState(1);

  useEffect(() => {
    const token = localStorage.getItem("token");
    if (cacheReviews.length > 0) return setCacheReviews(cacheReviews);

    const getData = async () => {
      const res = await fetch(
        `${DOMAIN}reviews?id=${teacherId}&role=teacher&page=${page}`,
        {
          headers: {
            ...
            >
              ...
              <div className="bg-[#F3F4F5] rounded-2xl p-8  mt-4 lg:mt-0 flex flex-col items-center text-center">
                ...
                <p className="text-[#424752] text-sm font-normal leading-relaxed">
                  جميع التقييمات صادرة من طلاب مسجلين بالفعل في الدورات
                  التعليمية لضمان المصداقية الكاملة
                </p>
              </div>
            </div>
          </div>
