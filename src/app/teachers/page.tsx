...
export default function TeachersPage() {
  ...
  const handleFilter = () => {
    const result = teachersData.filter((teacher) => {
      ...
      const matchSystem = system ? teacher.systems.includes(system) : true;

      const matchSubject = subject ? teacher.subject === subject : true;

      const matchGrade = grade ? teacher.stages.includes(grade) : true;
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          {isFiltered && (
            <button
              ...
              className="bg-[#ADD8E6] px-6 py-3 text-white relative overflow-hidden z-30 group hover:bg-sky-800 transition-all duration-500 rounded tracking-wider font-semibold cursor-pointer"
            >
              عرض الكل
              <svg
                className="absolute inset-0 left-0 top-0 fill-sky-300 -z-30 opacity-0 group-hover:opacity-100 group-hover:duration-300 group-hover:transition-all group-active:fill-sky-950 cursor-pointer"
          ...
          <div
            onClick={handleSort}
            className="flex items-center gap-2 text-slate-500 bg-white px-4 py-2 rounded-xl border border-slate-100 cursor-pointer hover:bg-slate-50 transition shadow-sm"
          >
