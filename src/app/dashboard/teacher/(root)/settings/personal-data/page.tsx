...
const profileSchema = z.object({
  ...
  bio: z
    .string()
    .min(10, "النبذة يجب ألا تقل عن 50 حرفاً")
...
const PersonalDataPage = () => {
  ...
  useEffect(() => {
    const getmedata = async () => {
      ...
      setValue("educationalStage", user.educationalStage);
      setValue("classRoom", user.teacher.educationalStage);
      setValue("studyMaterial", user.teacher.studyMaterial);
      setValue("qualification", user.teacher.qualification);
      setValue("experience", user.teacher.experience);
  ...
  const onSubmit = async (formData: ProfileFormData) => {
    ...
    const res = await axios.post(`${DOMAIN}users/${senderId}`, formData, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
